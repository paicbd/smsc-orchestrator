...
class HttpSubmitPreProcessTest {

    @Mock(strictness = Mock.Strictness.LENIENT)
    JedisCluster jedisCluster;
    @Mock
    AppProperties appProperties;
    HttpSubmitPreProcess httpSubmitPreProcess;

    @Test
    void startScheduler() {
        this.httpSubmitPreProcess = new HttpSubmitPreProcess(appProperties, jedisCluster);
        when(appProperties.getHttpWorkers()).thenReturn(10);
        when(appProperties.getHttpSubmitSmQueue()).thenReturn("http_message");
        ...
        assertDoesNotThrow(() -> httpSubmitPreProcess.startScheduler());
    }

    @Test
    void startSchedulerThrowsException() {
        this.httpSubmitPreProcess = new HttpSubmitPreProcess(null, jedisCluster);
        assertDoesNotThrow(() -> httpSubmitPreProcess.startScheduler());
    }
}